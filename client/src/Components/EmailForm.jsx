...
try {
      await axios.post("/form", {email: email});
...
<div className="col">
  <input type="email"
              id="email"
              name="email"
              value={email} className="form-control" placeholder="name@example.com" onChange={(e) => {setEmail(e.target.value)}}></input>
  </div>
  <div className="col">
 <button type="submit" className="btn btn-warning">{eng ? ("Sign up") : ("Pieteikties")}</button>
  </div>
  </div>
</form>)
:
